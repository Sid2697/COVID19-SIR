...
class Learner(object):
    ...
    def train(self):
        ...
        ax.set_title('SIR Model Predictions for ' + self.country, fontsize=12)
        ax.set_xlabel('Number of days from 22nd Jan 2020', fontsize=9)
        ax.set_ylabel('Number of people', fontsize=9)
        df = df.reset_index()
        df = df.drop(columns=['index'])
        df.plot(ax=ax)
        with open('images/SIR/{}.txt'.format(self.country), 'w') as file:
            file.write(f"country={self.country}, beta={beta:.8f}, gamma={gamma:.8f}, r_0:{(beta/gamma):.8f}")
        print(f"country={self.country}, beta={beta:.8f}, gamma={gamma:.8f}, r_0:{(beta/gamma):.8f}, Function value {optimal.fun}")
        fig.savefig(f"images/SIR/{self.country}_300.png")
